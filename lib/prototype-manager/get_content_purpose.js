...
const jsonFile = fs.readFileSync(path.resolve(__dirname, 'data', args.f));
...
function makeResponseHandler (basePath) {
  return function (jsonData) {
    ...
    if (idx < contentItems.length) {
      ...
      fs.writeFileSync(path.resolve(__dirname, `./data/content_purposes_for_${org}_content_items.json`), JSON.stringify(idContentPurposeMap, undefined, 2), { 'encoding': 'utf8' });
