...
const jsonFile = fs.readFileSync(path.resolve(__dirname, './content_data_2017_05_29-2018_05_29_top_1000.json'), { 'encoding': 'utf8' });
...
function makeResponseHandler (requestType, metric, content_id) {
    ...
    if (!(metric in data[content_id])) {
      data[content_id][metric] = {};
		...
		if (requestType === 'aggregation') {
			...
			data[content_id][metric].total = jsonData.total;
			totalDone = true;
		}
		else {
    ...
    if (itemIdx < items.length) {
      ...
      fs.writeFileSync(path.resolve(__dirname, './content_item_data_2017_05_29-2018_05_29.json'), JSON.stringify(data, undefined, 2), { 'encoding': 'utf8' });
    }
	};
};
