class Organisations {
  constructor () {
    this._mapping = [
      {
        "acronym": "fco",
        "name": "Foreign & Commonwealth Office"
      },
      {
        "acronym": "hmrc",
        "name": "HM Revenue & Customs"
      },
      {
        "acronym": "ea",
        "name": "Environment Agency"
      },
      {
        "acronym": "gds",
        "name": "Government Digital Service"
      },
      {
        "acronym": "dcms",
        "name": "Department for Digital, Culture, Media & Sport"
      },
      {
        "acronym": "dfe",
        "name": "Department for Education"
      },
      {
        "acronym": "beis",
        "name": "Department for Business, Energy & Industrial Strategy"
      },
      {
        "acronym": "phe",
        "name": "Public Health England"
      },
      {
        "acronym": "moj",
        "name": "Ministry of Justice"
      },
      {
        "acronym": "defra",
        "name": "Department for Environment, Food & Rural Affairs"
      },
      {
        "acronym": "dwp",
        "name": "Department for Work and Pensions"
      },
      {
        "acronym": "ho",
        "name": "Home Office"
      },
      {
        "acronym": "co",
        "name": "Cabinet Office"
      },
      {
        "acronym": "mod",
        "name": "Ministry of Defence"
      },
      {
        "acronym": "dfid",
        "name": "Department for International Development"
      },
      {
        "acronym": "mhclg",
        "name": "Ministry of Housing, Communities & Local Government"
      },
      {
        "acronym": "dit",
        "name": "Department for International Trade"
      },
      {
        "acronym": "ofsted",
        "name": "Ofsted"
      },
      {
        "acronym": "dvsa",
        "name": "Driver and Vehicle Standards Agency"
      },
      {
        "acronym": "dvla",
        "name": "Driver and Vehicle Licensing Agency"
      },
      {
        "acronym": "nctl",
        "name": "National College for Teaching and Leadership"
      },
      {
        "acronym": "dft",
        "name": "Department for Transport"
      },
      {
        "acronym": "ofqual",
        "name": "Ofqual"
      },
      {
        "acronym": "ons",
        "name": "Office for National Statistics"
      },
      {
        "acronym": "mca",
        "name": "Maritime and Coastguard Agency"
      },
      {
        "acronym": "cma",
        "name": "Competition and Markets Authority"
      },
      {
        "acronym": "rpa",
        "name": "Rural Payments Agency"
      },
      {
        "acronym": "voa",
        "name": "Valuation Office Agency"
      },
      {
        "acronym": "dhsc",
        "name": "Department of Health and Social Care"
      },
      {
        "acronym": "apha",
        "name": "Animal and Plant Health Agency"
      },
      {
        "acronym": "fsa",
        "name": "Food Standards Agency"
      },
      {
        "acronym": "nda",
        "name": "Nuclear Decommissioning Authority"
      },
      {
        "acronym": "mhra",
        "name": "Medicines and Healthcare Products Regulatory Agency"
      },
      {
        "acronym": "hmt",
        "name": "HM Treasury"
      },
      {
        "acronym": "certification-office",
        "name": "Certification Officer"
      },
      {
        "acronym": "dexeu",
        "name": "Department for Exiting the European Union"
      },
      {
        "acronym": "ukef",
        "name": "UK Export Finance"
      },
      {
        "acronym": "sta",
        "name": "Standards and Testing Agency"
      },
      {
        "acronym": "yjb",
        "name": "Youth Justice Board for England and Wales"
      },
      {
        "acronym": "nio",
        "name": "Northern Ireland Office"
      },
      {
        "acronym": "cac-pensions",
        "name": "Central Arbitration Committee"
      },
      {
        "acronym": "cspl",
        "name": "Committee on Standards in Public Life"
      },
      {
        "acronym": "ch",
        "name": "Companies House"
      },
      {
        "acronym": "ccs",
        "name": "Crown Commercial Service"
      },
      {
        "acronym": "laa",
        "name": "Legal Aid Agency"
      },
      {
        "acronym": "maib",
        "name": "Marine Accident Investigation Branch"
      },
      {
        "acronym": "oisc",
        "name": "Office of the Immigration Services Commissioner"
      },
      {
        "acronym": "opg",
        "name": "Office of the Public Guardian"
      },
      {
        "acronym": "ukho ",
        "name": "UK Hydrographic Office"
      },
      {
        "acronym": "ukvi",
        "name": "UK Visas and Immigration"
      },
      {
        "acronym": "no10",
        "name": "Prime Minister's Office, 10 Downing Street"
      },
      {
        "acronym": "wales",
        "name": "Office of the Secretary of State for Wales"
      },
      {
        "acronym": "aaib",
        "name": "Air Accidents Investigation Branch"
      },
      {
        "acronym": "cnc",
        "name": "Civil Nuclear Constabulary"
      },
      {
        "acronym": "dit dso",
        "name": "Department for International Trade Defence & Security Organisation"
      },
      {
        "acronym": "ges",
        "name": "Government Economic Service"
      },
      {
        "acronym": "gca",
        "name": "Groceries Code Adjudicator"
      },
      {
        "acronym": "hmpps",
        "name": "Her Majesty’s Prison and Probation Service"
      },
      {
        "acronym": "ic-inspector",
        "name": "Independent Chief Inspector of Borders and Immigration"
      },
      {
        "acronym": "pins",
        "name": "Planning Inspectorate"
      },
      {
        "acronym": "pca",
        "name": "Pubs Code Adjudicator"
      },
      {
        "acronym": "ssro",
        "name": "Single Source Regulations Office"
      },
      {
        "acronym": "ukaea",
        "name": "UK Atomic Energy Authority"
      },
      {
        "acronym": "vmd",
        "name": "Veterinary Medicines Directorate"
      },
      {
        "acronym": "ago",
        "name": "Attorney General's Office"
      },
      {
        "acronym": "gad",
        "name": "Government Actuary's Department"
      },
      {
        "acronym": "cica",
        "name": "Criminal Injuries Compensation Authority"
      },
      {
        "acronym": "ddat",
        "name": "Digital, Data and Technology Profession"
      },
      {
        "acronym": "dbs",
        "name": "Disclosure and Barring Service"
      },
      {
        "acronym": "gad",
        "name": "Government Actuary's Department"
      },
      {
        "acronym": "gls",
        "name": "Government Legal Service"
      },
      {
        "acronym": "healthcareuk",
        "name": "Healthcare UK"
      },
      {
        "acronym": "hmps",
        "name": "HM Prison Service"
      },
      {
        "acronym": "ipa",
        "name": "Infrastructure and Projects Authority"
      },
      {
        "acronym": "ifa",
        "name": "Institute for Apprenticeships"
      },
      {
        "acronym": "ome",
        "name": "Office of Manpower Economics"
      },
      {
        "acronym": "ots",
        "name": "Office of Tax Simplification"
      },
      {
        "acronym": "cic regulator",
        "name": "Office of the Regulator of Community Interest Companies"
      },
      {
        "acronym": "parole-board",
        "name": "Parole Board"
      },
      {
        "acronym": "rwm",
        "name": "Radioactive Waste Management"
      },
      {
        "acronym": "slc",
        "name": "Student Loans Company"
      },
      {
        "acronym": "ukcb",
        "name": "UK Co-ordinating Body"
      },
      {
        "acronym": "ags",
        "name": "Office of the Advocate General for Scotland"
      },
      {
        "acronym": "cnpa",
        "name": "Civil Nuclear Police Authority"
      },
      {
        "acronym": "deca",
        "name": "Defence Electronics and Components Agency"
      },
      {
        "acronym": "geo",
        "name": "Government Equalities Office"
      },
      {
        "acronym": "giaa",
        "name": "Government Internal Audit Agency"
      },
      {
        "acronym": "lpc",
        "name": "Low Pay Commission"
      },
      {
        "acronym": "maa",
        "name": "Military Aviation Authority"
      },
      {
        "acronym": "nactso",
        "name": "National Counter Terrorism Security Office"
      },
      {
        "acronym": "ndg",
        "name": "National Data Guardian"
      },
      {
        "acronym": "ssac",
        "name": "Social Security Advisory Committee"
      },
      {
        "acronym": "ukhmf",
        "name": "UK Holocaust Memorial Foundation"
      },
      {
        "acronym": "leader-commons",
        "name": "Office of the Leader of the House of Commons"
      },
      {
        "acronym": "leader-commons",
        "name": "Office of the Leader of the House of Commons"
      },
      {
        "acronym": "gld",
        "name": "Government Legal Department"
      },
      {
        "acronym": "arsac",
        "name": "Administration of Radioactive Substances Advisory Committee"
      },
      {
        "acronym": "abrhp",
        "name": "Advisory Board on Registration of Homeopathic Products"
      },
      {
        "acronym": "aprhai",
        "name": "Advisory Committee on Antimicrobial Prescribing, Resistance and Healthcare Associated Infection"
      },
      {
        "acronym": "acbs",
        "name": "Advisory Committee on Borderline Substances"
      },
      {
        "acronym": "acoba",
        "name": "Advisory Committee on Business Appointments"
      },
      {
        "acronym": "accea",
        "name": "Advisory Committee on Clinical Excellence Awards"
      },
      {
        "acronym": "acco",
        "name": "Advisory Committee on Conscientious Objectors"
      },
      {
        "acronym": "acre",
        "name": "Advisory Committee on Releases to the Environment"
      },
      {
        "acronym": "sabto",
        "name": "Advisory Committee on the Safety of Blood, tissues and organs"
      },
      {
        "acronym": "acjp",
        "name": "Advisory Committees on Justices of the Peace"
      },
      {
        "acronym": "acmd",
        "name": "Advisory Council on the misuse of drugs"
      },
      {
        "acronym": "agh",
        "name": "Advisory Group on Hepatitis"
      },
      {
        "acronym": "agmm",
        "name": "Advisory Group on Military Medicine"
      },
      {
        "acronym": "airports-commission",
        "name": "Airports Commission"
      },
      {
        "acronym": "ahwb",
        "name": "Animal Health and Welfare Board for England *"
      },
      {
        "acronym": "animals-in-science-committee",
        "name": "Animals in Science Committee"
      },
      {
        "acronym": "afprb",
        "name": "Armed Forces' Pay Review Board "
      },
      {
        "acronym": "bc",
        "name": "Biometrics Commissioner"
      },
      {
        "acronym": "bv",
        "name": "Bona Vacantia Division of Treasury Solicitors"
      },
      {
        "acronym": "bf",
        "name": "Border Force"
      },
      {
        "acronym": "bcms",
        "name": "British Cattle Movement Service"
      },
      {
        "acronym": "british-hallmarking-council",
        "name": "British Hallmarking Council "
      },
      {
        "acronym": "cac compensation",
        "name": "Central Advisory Committee on Compensation"
      },
      {
        "acronym": "ccav",
        "name": "Centre for Connected and Autonomous Vehicles"
      },
      {
        "acronym": "cefas",
        "name": "Centre for Environment, Fisheries and Aquaculture Science "
      },
      {
        "acronym": "cfra",
        "name": "Chief Fire and Rescue Adviser"
      },
      {
        "acronym": "cprc",
        "name": "Civil Procedure Rules Committee"
      },
      {
        "acronym": "chm",
        "name": "Commission on Human Medicines"
      },
      {
        "acronym": "cfp",
        "name": "Committee on Fuel Poverty"
      },
      {
        "acronym": "coare",
        "name": "Committee on Medical Aspects of Radiation in the Environment"
      },
      {
        "acronym": "comeap",
        "name": "Committee on Medical Effects of Air Pollutants"
      },
      {
        "acronym": "com ",
        "name": "Committee on Mutagenicity of Chemicals in Food, Consumer Products and the Environment "
      },
      {
        "acronym": "corwm",
        "name": "Committee on Radioactive Waste Management"
      },
      {
        "acronym": "csc",
        "name": "Commonwealth Scholarship Commission in the UK"
      },
      {
        "acronym": "cic-regulator",
        "name": "Community Interest Companies Regulator"
      },
      {
        "acronym": "frca",
        "name": "Council for Reserve Forces and Cadets Associations"
      },
      {
        "acronym": "cst",
        "name": "Council for Science and Technology"
      },
      {
        "acronym": "cfo",
        "name": "Court Funds Office"
      },
      {
        "acronym": "des",
        "name": "Defence Equipment and Support"
      },
      {
        "acronym": "dio",
        "name": "Defence Infrastructure Organisation"
      },
      {
        "acronym": "dnsc",
        "name": "Defence Nuclear Safety Committee"
      },
      {
        "acronym": "dsa ",
        "name": "Defence Safety Authority"
      },
      {
        "acronym": "dstl",
        "name": "Defence Science & Technology"
      },
      {
        "acronym": "egac",
        "name": "Department for International Trade: Export Guarantees Advisory Council"
      },
      {
        "acronym": "ddrb",
        "name": "Review Body on Doctors' and Dentists' Remuneration"
      },
      {
        "acronym": "ssrb",
        "name": "Review Body on Senior Salaries"
      },
      {
        "acronym": "strb",
        "name": "School Teachers' Review Body"
      },
      {
        "acronym": "chpqa",
        "name": "Department of Energy and Climate Change: Combined Heat and Power Quality Assurance Programme"
      },
      {
        "acronym": "eaga",
        "name": "Department of Health: Expert Advisory Group on AIDS"
      },
      {
        "acronym": "jcvi",
        "name": "Department of Health: Joint Committee on Vaccination and Immunisation"
      },
      {
        "acronym": "dpec",
        "name": "Disabled Persons Transport Advisory Committee"
      },
      {
        "acronym": "dvs ",
        "name": "District Valuer Services"
      },
      {
        "acronym": "esfa",
        "name": "Education and Skills Funding Agency"
      },
      {
        "acronym": "ecju",
        "name": "Export Control Joint Unit"
      },
      {
        "acronym": "egac",
        "name": "Export Guarantees Advisory Council"
      },
      {
        "acronym": "fprc",
        "name": "Family Procedure Rule Committee"
      },
      {
        "acronym": "fso",
        "name": "Financial Services Organisation"
      },
      {
        "acronym": "fstib",
        "name": "Financial Services Trade and Investment Board"
      },
      {
        "acronym": "gcf",
        "name": "Government Commercial Function"
      },
      {
        "acronym": "gcs",
        "name": "Government Communication Service"
      },
      {
        "acronym": "www.gov.uk/government/groups/government-decontamination-service",
        "name": "Government Decontamination Service"
      },
      {
        "acronym": "gds",
        "name": "Government Digital Service  "
      },
      {
        "acronym": "gld ",
        "name": "Government Legal Department "
      },
      {
        "acronym": "go-science",
        "name": "Government Office for Science"
      },
      {
        "acronym": "gpa",
        "name": "Government Property Agency"
      },
      {
        "acronym": "government-whips-office",
        "name": "Government Whips Office"
      },
      {
        "acronym": "hs2",
        "name": "High Speed Two Limited"
      },
      {
        "acronym": "hmcts",
        "name": "HM Courts & Tribunals Service"
      },
      {
        "acronym": "hmnao",
        "name": "HM Nautical Alamanac Office"
      },
      {
        "acronym": "hmpo",
        "name": "HM Passport Office"
      },
      {
        "acronym": "fsr",
        "name": "Home Office: Forensic Science Regulator"
      },
      {
        "acronym": "scc",
        "name": "Surveillance Camera Commisioner"
      },
      {
        "acronym": "homes-england",
        "name": "Homes England "
      },
      {
        "acronym": "ie",
        "name": "Immigration Enforcement"
      },
      {
        "acronym": "iaap",
        "name": "Independent Agricultural Appeals Panel"
      },
      {
        "acronym": "iasc",
        "name": "Independent Anti-slavery Commissioner"
      },
      {
        "acronym": "ice",
        "name": "Independent Case Examiner"
      },
      {
        "acronym": "ifrp",
        "name": "Independent Family Returns Panel"
      },
      {
        "acronym": "imeg",
        "name": "Independent Medical Expert Group"
      },
      {
        "acronym": "irp",
        "name": "Independent Reconfiguration Panel"
      },
      {
        "acronym": "idab",
        "name": "Industrial Development Advisory Board"
      },
      {
        "acronym": "iiac",
        "name": "Industrial Injuries Advisory Council"
      },
      {
        "acronym": "ico",
        "name": "Information Commissioner's office"
      },
      {
        "acronym": "insolvency-service",
        "name": "The Insolvency Service"
      },
      {
        "acronym": "irc",
        "name": "Insolvency Service: Insolvency Rules Committee"
      },
      {
        "acronym": "ipo",
        "name": "Intellectual Property Office"
      },
      {
        "acronym": "jfc",
        "name": "Joint Forces Command"
      },
      {
        "acronym": "jaco",
        "name": "Judicial Appointments and Conduct Ombudsman"
      },
      {
        "acronym": "lrrc",
        "name": "Land Registration Rule Committee"
      },
      {
        "acronym": "llwr",
        "name": "Low Level Waste Repository Ltd "
      },
      {
        "acronym": "mmo",
        "name": "Marine Management Organisation MMO"
      },
      {
        "acronym": "csd",
        "name": "Medicines and Healthcare Regulatory Agency: Committee on the Safety of Devices"
      },
      {
        "acronym": "hmac",
        "name": "Medicines and Healthcare Regulatory Agency: Herbal Medicines Advisory Committee"
      },
      {
        "acronym": "mac",
        "name": "Migration Advisory Committee"
      },
      {
        "acronym": "ncarrb",
        "name": "National Crime Agency Remuneration Review Body"
      },
      {
        "acronym": "nps",
        "name": "National Probation Service"
      },
      {
        "acronym": "nsc",
        "name": "National Security Council (CO and FCO parents)"
      },
      {
        "acronym": "www.gov.uk/government/groups/natural-capital-committee",
        "name": "Natural Capital Committee"
      },
      {
        "acronym": "nhsprb",
        "name": "NHS Pay Review Body"
      },
      {
        "acronym": "nlfab",
        "name": "Nuclear Liabilities Financing Assurance Board"
      },
      {
        "acronym": "nrac",
        "name": "Nuclear Research Advisory Council"
      },
      {
        "acronym": "ocs",
        "name": "Office for Civil Society"
      },
      {
        "acronym": "odi",
        "name": "Office for Disability Issues"
      },
      {
        "acronym": "ols",
        "name": "Office for Life Sciences"
      },
      {
        "acronym": "olev",
        "name": "Office for Low Emission Vehicles "
      },
      {
        "acronym": "ofsi",
        "name": "Office of Financial Sanctions Implementation"
      },
      {
        "acronym": "ofqual",
        "name": "Office of Qualifications and Examinations Regulation"
      },
      {
        "acronym": "opc",
        "name": "Office of the Parliamentary Counsel"
      },
      {
        "acronym": "osa",
        "name": "Office of the Schools Adjudicator"
      },
      {
        "acronym": "ospt",
        "name": "Official Solicitor and Public Trustee"
      },
      {
        "acronym": "opa",
        "name": "Oil and Pipelines Agency"
      },
      {
        "acronym": "pvst",
        "name": "Plant Varieties and Seeds Tribunal"
      },
      {
        "acronym": "pat",
        "name": "Police Discipline Appeals Tribunal"
      },
      {
        "acronym": "pab",
        "name": "Police Advisory Board for England and Wales"
      },
      {
        "acronym": "prrb",
        "name": "Police Remuneration Review Body"
      },
      {
        "acronym": "psvi",
        "name": "Preventing Sexual Violence Initiative"
      },
      {
        "acronym": "psprb",
        "name": "Prison Services Pay Review Body"
      },
      {
        "acronym": " ",
        "name": "Probation Services"
      },
      {
        "acronym": " ",
        "name": "Probation Trusts"
      },
      {
        "acronym": "phe",
        "name": "Public Health England: UK Atmospheric Dispersion Modelling Liaison Committee"
      },
      {
        "acronym": "qhm",
        "name": "Queen's Harbour Master"
      },
      {
        "acronym": "raib",
        "name": "Rail Accidents Investigation Branch"
      },
      {
        "acronym": "rpc",
        "name": "Regulatory Policy Committee"
      },
      {
        "acronym": "rmac",
        "name": "Royal Mint Advisory Committee"
      },
      {
        "acronym": "rdpe ",
        "name": "Rural Development Programme for England Network"
      },
      {
        "acronym": "sascmill",
        "name": "Science Advisory Committee on the Medical Implications of Less-Lethal Weapons"
      },
      {
        "acronym": "sacn",
        "name": "Science Advisory Council"
      },
      {
        "acronym": "sacn",
        "name": "Scientific Advisory Committee on Nutrition"
      },
      {
        "acronym": "qcf",
        "name": "Skills Funding Agency: Qualifications and Credit Framework"
      },
      {
        "acronym": "smc",
        "name": "Social Mobility Commission"
      },
      {
        "acronym": "tra ",
        "name": "Teaching Regulation Agency"
      },
      {
        "acronym": "tab",
        "name": "Technical Advisory Board"
      },
      {
        "acronym": "traffic-commissioners",
        "name": "Traffic Commissioners"
      },
      {
        "acronym": "tpc",
        "name": "Tribunal Procedure Committee"
      },
      {
        "acronym": "dmo",
        "name": "UK Debt Management Office"
      },
      {
        "acronym": "ukef",
        "name": "UK Export Finance "
      },
      {
        "acronym": "ukfi",
        "name": "UK Financial Investments Limited"
      },
      {
        "acronym": "vapc",
        "name": "Veterans Advisory and Pensions Committees (x13)"
      },
      {
        "acronym": "vpc",
        "name": "Veterinary Products Committee"
      }
    ];
  }

  get acronyms () {
      return this._mapping.map(entry => entry.acronym);
    }

  get names () {
    return this._mapping.map(entry => entry.name);
  }

  getNameForAcronym (acronym) {
    return this._mapping.filter(entry => { return entry.acronym === acronym; })[0].name;
  }

  getAcronymForName (name) {
    return this._mapping.filter(entry => { return entry.name === name; })[0].acronym;
  }

  asHash () {
    return this._mapping.map(entry => {
      const result = {};

      result[entry.acronym] = entry.name;
    });
  }

  putOrgAtTop (orgAcronym) {
    let idx;
    let row;

    this._mapping.forEach((entry, entryIdx) => { if (entry.acronym === orgAcronym) { idx = entryIdx; } });

    row = Object.assign({}, this._mapping[idx]);
    this._mapping.splice(idx, 1);
    this._mapping.unshift(row);
  }
};

module.exports = new Organisations;
