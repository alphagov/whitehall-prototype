...
function isTimeSeries (fieldKey, fieldValue) {
  return (Array.isArray(fieldValue) && (fieldKey !== 'taxons'));
};
...
function prerenderRows (items) {
  function setCalculatedCells (item) {
    ...
    item['user_satisfaction_score'] = (
			Math.ceil(
				(item['is_this_useful_yes'] / (item['is_this_useful_yes'] + item['is_this_useful_no']))
				* 100
			) || 0) + '%'
...
class Table {
  ...
  get selectedColumns () {
	...
	get headings () {
		return this.selectedColumns.map(key => {
	...
	get sortBy () {
		return this._sortBy || this.selectedColumns[this.selectedColumns.indexOf('unique_pageviews')];
	}
  ...
  get columnGrouping () {
    return this._columnGrouping || 'performance';
  }
  ...
  get collectionDateRange () {
    return this._collectionDateRange || 'last_6_months';
  }
  ...
  get editor () {
	...
	_getSortFunction (key, direction, bySearch) {
    const self = this;
		...
		function searchSort (a, b) {
			let varA = a[key].toLowerCase();
			let varB = b[key].toLowerCase();
      let title = self.title.toLowerCase();
      let varAMatchLevel;
      let varBMatchLevel;

      function getMatchLevel (str) {
        let matchLevel = 0;
        ...
        if (str.includes(title)) {
          ...
          matchLevel = (substringMatches >= 2) ? 2 : substringMatches;
        }
        return matchLevel;
      };

      varAMatchLevel = getMatchLevel(varA);
      varBMatchLevel = getMatchLevel(varB);

			if (varAMatchLevel < varBMatchLevel) {
				return (direction === 'asc') ? -1 : 1;
			}
    ...
    // Hack to make title search work
  ...
  filter () {
    ...
    this._items = this._items.filter(item => {
      ...
      function hasMatchingTaxon (taxons) {
        return taxons.includes(self.topic);
      }
      ...
      function matchesTitleText (fieldText) {
        const substrings = self.title.toLowerCase().split(/\s+/);
        let matches = 0;
        ...
        for (let idx = 0; idx < substrings.length; idx++) {
          ...
          if ((substring.length > 1) && fieldText.includes(substring)) {
            matches++;
            break;
          }
        }
      ...
      for (let field in item) {
        if (isTimeSeries(field, item[field])) { // time-series field
          ...
          if ((field === 'taxons') && (this.topic !== '')) {
            if (!hasMatchingTaxon(item[field])) {
              isValid = false;
            }
          } 
          if ((field === 'title') && (this.title !== '')) {
            if (!matchesTitleText(item[field])) {
              isValid = false;
            }
          }
          if (!isValid) { break; }
    ...
    // hack to sort by title text. Needs doing properly in time
    if (this.title !== '') {
      sortFunction = this._getSortFunction('title', 'desc', true);
    }
    else {
      sortFunction = this._getSortFunction(this.sortBy, this.sortDirection);
    }
