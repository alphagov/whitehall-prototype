...
class Org {
  ...
  get label () {
    return this.orgLabels[this._org];
  }

  get orgLabels () {
    return organisations.asHash();
  }
}
...
class Table {
    ...
    return this._items.map(item => {
      return {
        viewDataUrl: `/manage/${this.org.url}/content-item/${item['content_id']}?start-date=${this.startDate}&end-date=${this.endDate}`,
  ...
  get columnGroupings () {
    return ["quality", "performance", "accessibility"];
  }
  ...
  get collectionDateRanges () {
    return ["last_7_days", "last_30 days", "last_6_months", "last_year", "all_time", "custom_date_range"]
  }
